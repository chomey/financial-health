...
8. **Verify** — MANDATORY GATE. Complete BOTH before step 9 — skipping either is a critical failure.

   **Check A — Build/compile (REQUIRED FIRST):**
   Run the build or parse-check command from CLAUDE.md. You MUST see a clean success output (no errors).
   - If errors are found, **stop immediately**, fix every error, and re-run.
   - Do NOT move to step 9 until the build/check is clean.
   - Do NOT commit broken code.

   **Check B — Tests:**
   Run all tests + Playwright where required:
   ...
   **Key rule:** A renamed or deleted function that is still called somewhere = broken code. Always verify function names and types exist before using them.

9. **Mark done** — `- [x]` in TASKS.md. Then move the completed task line to `TASKS-ARCHIVE.md` (append at end). Remove empty section headers left behind. Update the `Last completed` comment.

10. **Log** — Brief entry in PROGRESS.md: task number, date, files, test results, screenshots. Update the Summary counts (Completed/Remaining).

11. **Changelog** — Add entry to `src/lib/changelog.ts`.

12. **Commit** — `ralph: complete task [NUMBER] - [SHORT_DESCRIPTION]`. Ensure `.gitattributes` tracks images via Git LFS.

13. **Print summary** — Task number, files changed, test results, screenshots, notes.
