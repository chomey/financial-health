...
describe("calculateMedicareLevy", () => {
  ...
  it("phases in between $27,222 and $34,027", () => {
    // At $29,000: 10% of ($29,000 - $27,222) = $177.80
    expect(calculateMedicareLevy(29_000, 2025)).toBeCloseTo(178, 0);
  });

  it("full 2% above shade-out ($34,027)", () => {
  ...
  it("uses 2026 thresholds when year >= 2026", () => {
    ...
    // $27,500 is above 2025 threshold ($27,222) → phase-in
...
describe("AU computeTax — 2026 tax year", () => {
  it("uses unchanged 2026 brackets", () => {
    // AU does not index resident tax brackets annually; 2026 federal tax matches 2025.
    const result2025 = computeTax(100_000, "employment", "AU", "NSW", 2025);
    const result2026 = computeTax(100_000, "employment", "AU", "NSW", 2026);
    expect(result2026.totalTax).toBe(result2025.totalTax);
  });
