...
describe("changelog data", () => {
  it("contains entries for all 70 completed tasks", () => {
    expect(CHANGELOG.length).toBe(70);
  });
  ...
  it("covers versions 1 through 70", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(70);
    for (let i = 1; i <= 70; i++) {
