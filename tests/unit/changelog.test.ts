...
describe("changelog data", () => {
  it("contains entries for all 72 completed tasks", () => {
    expect(CHANGELOG.length).toBe(72);
  });
  ...
  it("covers versions 1 through 72", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(72);
    for (let i = 1; i <= 72; i++) {
