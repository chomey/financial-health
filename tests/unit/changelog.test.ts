...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[1].entries.length).toBe(8);
    expect(milestones[1].entries.every((e) => e.version >= 69 && e.version <= 76)).toBe(true);
    // Withdrawal Tax Modeling: 62-68
    expect(milestones[2].milestone).toBe("Withdrawal Tax Modeling");
    expect(milestones[2].entries.length).toBe(7);
    expect(milestones[2].entries.every((e) => e.version >= 62 && e.version <= 68)).toBe(true);
    // Multi-Currency: 56-61
    expect(milestones[3].milestone).toBe("Multi-Currency Support");
    expect(milestones[3].entries.length).toBe(6);
    expect(milestones[3].entries.every((e) => e.version >= 56 && e.version <= 61)).toBe(true);
    // Foundation: 1-14
    expect(milestones[8].milestone).toBe("Foundation & Initial Build");
    expect(milestones[8].entries.length).toBe(14);
    expect(milestones[8].entries.every((e) => e.version >= 1 && e.version <= 14)).toBe(true);
  });
