...
describe("changelog data", () => {
  it("contains entries for completed tasks (167 entries, versions 1-167)", () => {
    expect(CHANGELOG.length).toBe(167);
  });
  ...
  it("covers versions 1 through 167", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(167);
    for (let i = 1; i <= 167; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(10); // 158-167
