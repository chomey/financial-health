...
describe("changelog data", () => {
  it("contains entries for completed tasks (165 entries, versions 1-165)", () => {
    expect(CHANGELOG.length).toBe(165);
  });
  ...
  it("covers versions 1 through 165", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(165);
    for (let i = 1; i <= 165; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(8); // 158, 159, 160, 161, 162, 163, 165, 165
