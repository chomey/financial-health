...
describe("changelog data", () => {
  it("contains entries for completed tasks (164 entries, versions 1-164)", () => {
    expect(CHANGELOG.length).toBe(164);
  });
  ...
  it("covers versions 1 through 164", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(164);
    for (let i = 1; i <= 164; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(7); // 158, 159, 160, 161, 162, 163, 164
