...
describe("changelog data", () => {
  it("contains entries for all 87 completed tasks", () => {
    expect(CHANGELOG.length).toBe(87);
  });
  ...
  it("covers versions 1 through 87", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(87);
    for (let i = 1; i <= 87; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(5); // 83, 84, 85, 86, 87
    ...
    expect(milestones[1].entries.length).toBe(4);
    expect(milestones[1].entries.every((e) => e.version >= 79 && e.version <= 82)).toBe(true);
    // Spotlight Dimming System: 77-78
    expect(milestones[2].milestone).toBe("Spotlight Dimming System");
    expect(milestones[2].entries.length).toBe(2);
    expect(milestones[2].entries.every((e) => e.version >= 77 && e.version <= 78)).toBe(true);
    // Data Flow Visualization: 69-76
    expect(milestones[3].milestone).toBe("Data Flow Visualization");
    expect(milestones[3].entries.length).toBe(8);
    expect(milestones[3].entries.every((e) => e.version >= 69 && e.version <= 76)).toBe(true);
    // Withdrawal Tax Modeling: 62-68
    expect(milestones[4].milestone).toBe("Withdrawal Tax Modeling");
    expect(milestones[4].entries.length).toBe(7);
    expect(milestones[4].entries.every((e) => e.version >= 62 && e.version <= 68)).toBe(true);
    // Multi-Currency: 56-61
    expect(milestones[5].milestone).toBe("Multi-Currency Support");
    expect(milestones[5].entries.length).toBe(6);
    expect(milestones[5].entries.every((e) => e.version >= 56 && e.version <= 61)).toBe(true);
    // Foundation: 1-14
    expect(milestones[10].milestone).toBe("Foundation & Initial Build");
    expect(milestones[10].entries.length).toBe(14);
    expect(milestones[10].entries.every((e) => e.version >= 1 && e.version <= 14)).toBe(true);
  });
