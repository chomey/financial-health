...
describe("changelog data", () => {
  it("contains entries for all 65 completed tasks", () => {
    expect(CHANGELOG.length).toBe(65);
  });
  ...
  it("covers versions 1 through 65", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(65);
    for (let i = 1; i <= 65; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("contains all 65 entries across all groups", () => {
    const milestones = getChangelogByMilestone();
    const totalEntries = milestones.reduce((sum, m) => sum + m.entries.length, 0);
    expect(totalEntries).toBe(65);
  });
