...
describe("changelog data", () => {
  it("contains entries for completed tasks (143 entries, versions 1-143)", () => {
    expect(CHANGELOG.length).toBe(143);
  });
  ...
  it("covers versions 1 through 143", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(143);
    for (let i = 1; i <= 143; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(4); // 140, 141, 142, and 143
