...
describe("changelog data", () => {
  it("contains entries for completed tasks (168 entries, versions 1-168)", () => {
    expect(CHANGELOG.length).toBe(168);
  });
  ...
  it("covers versions 1 through 168", () => {
    const versions = CHANGELOG.map((e) => e.version).sort((a, b) => a - b);
    expect(versions[0]).toBe(1);
    expect(versions[versions.length - 1]).toBe(168);
    for (let i = 1; i <= 168; i++) {
...
describe("getChangelogByMilestone", () => {
  ...
  it("groups entries correctly by milestone range", () => {
    ...
    expect(milestones[0].entries.length).toBe(11); // 158-168
