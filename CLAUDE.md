...
- **Screenshot capture is opt-in and task-scoped** — `captureScreenshot()` only writes files when `CAPTURE_SCREENSHOTS=1` is set. Always set `CAPTURE_TASK=<N>` alongside it to limit writes to only the current task's screenshots (filenames containing `task-<N>`). This prevents the full suite from overwriting other tasks' screenshots. Example: `CAPTURE_SCREENSHOTS=1 CAPTURE_TASK=102 npx playwright test tests/e2e/my-test.spec.ts`.
